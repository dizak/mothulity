...
mothur '#set.current(processors=2, shared=analysis_travis_job.shared); sub.sample(shared=current)'
...
cd ../beta
mothur '#set.current(processors=2, shared=analysis_travis_job.0.03.subsample.shared); dist.shared(shared=current, calc=thetayc-jclass, output=square)'
